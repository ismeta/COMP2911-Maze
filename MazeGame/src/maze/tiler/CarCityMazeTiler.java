...
/**
 * Sets the images for tiles in the maze according to a "car in the city" style
 * theme.
 */
public class CarCityMazeTiler implements MazeTiler {
	/**
	 * @param tiles
	 *            the tiles representing the maze
	 * @param size
	 *            the number of tiles for width/height of the maze
	 * @param height
	 *            height of the maze (in pixels)
	 * @param width
	...
	public CarCityMazeTiler(MazeTile[][] tiles, int size, int height, int width) {
		...
		for (int i = 0; i < CarMazeCityTileType.values().length; i++) {
			try {
				tileImages[i] = ImageIO.read(new File("images/tiles/tile_" + i
						+ ".png"));
			} catch (IOException e) {
	...
	public void tileMaze() {
		this.image = new BufferedImage(this.mazeWidth, this.mazeHeight,
		...
		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {
				/*
				 * tile it differently depending on whether the current tile is
				 * a wall or not
				 */
				if (!tiles[i][j].isWall()) {
	 ...
	 * 
	 * @param imageG2D
	 * 			  the graphics instance to draw the images on
	 * @param i
	 *            which row to set the image for
	 * @param j
	 *            which column to set the image for
	 */
	private void setWallImage(Graphics2D imageG2D, int i, int j) {
		...
		/* Pick either house, building or tree pictures based on location */
		Image img = null;
		if (i % 2 == 0) {
	...
	/**
	 * Set the image for a tile based on the tiles around it - i.e. what kind of
	 * intersection it is. Rotates the image based on where the walls are.
	 * 
	 * @param imageG2D
	 * @param i
	 *            the row of the tile
	 ...
	 * @param neighbours
	 *            the number of non-wall neighbours the tile has
	 */
	private void setTileImage(Graphics2D imageG2D, int i, int j, int neighbours) {
		...
		case 1:
			if (j < size - 1 && !tiles[i][j + 1].isWall()) {
				tileImage = tileImages[CarMazeCityTileType.DEAD_END_HORIZONTAL
						.ordinal()];
				flipX = true;
				...
				tileImage = tileImages[CarMazeCityTileType.DEAD_END_VERTICAL
						.ordinal()];
				flipY = true;
		 ...
		 */
		case 2:
			if ((j > 0 && j < size - 1) && !tiles[i][j - 1].isWall()
					&& !tiles[i][j + 1].isWall()) {
				tileImage = tileImages[CarMazeCityTileType.STRAIGHT_HORIZONTAL
						.ordinal()];
			} else if ((i > 0 && i < size - 1) && !tiles[i + 1][j].isWall()
					&& !tiles[i - 1][j].isWall()) {
				tileImage = tileImages[CarMazeCityTileType.STRAIGHT_VERTICAL
						.ordinal()];
			} else if (i > 0 && j > 0 && !tiles[i - 1][j].isWall()
					&& !tiles[i][j - 1].isWall()) {
				tileImage = tileImages[CarMazeCityTileType.RIGHT_ANGLE
						.ordinal()];
				flipX = true;
			} else if (i > 0 && j < size - 1 && !tiles[i - 1][j].isWall()
					&& !tiles[i][j + 1].isWall()) {
				tileImage = tileImages[CarMazeCityTileType.RIGHT_ANGLE
						.ordinal()];
			} else if (i < size - 1 && j < size - 1
					&& !tiles[i + 1][j].isWall() && !tiles[i][j + 1].isWall()) {
				tileImage = tileImages[CarMazeCityTileType.RIGHT_ANGLE
						.ordinal()];
				flipY = true;
			} else {
		...
		case 3:
			if (j == size - 1 || (j < size - 1 && tiles[i][j + 1].isWall())) {
				tileImage = tileImages[CarMazeCityTileType.THREE_INTERSECT_VERTICAL
						.ordinal()];
				...
				flipX = true;
					...
					|| (i < size - 1 && tiles[i + 1][j].isWall())) {
				tileImage = tileImages[CarMazeCityTileType.THREE_INTERSECT_HORIZONTAL
						.ordinal()];
		...
		case 4:
			...
			break;
		default:
			throw new RuntimeException("illegal number of neighbours: "
					+ neighbours);
		}
		...
		if (flipX && !flipY) {
			...
			imageG2D.drawImage(tileImage, x + width, y + height, -width,
					-height, null);
		} else if (!flipX && flipY) {
	...
	private enum CarMazeCityTileType {
		EVERYTHING, THREE_INTERSECT_HORIZONTAL, THREE_INTERSECT_VERTICAL, RIGHT_ANGLE, STRAIGHT_HORIZONTAL, STRAIGHT_VERTICAL, DEAD_END_HORIZONTAL, DEAD_END_VERTICAL
	}
